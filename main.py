...
def main(
    ...
    graphdef, state = nnx.split((fb_model, fb_model_target, actor, fb_optimizer, actor_optimizer))
    ...
    for t in range(0, train_time_step, log_interval):
        ...
        (key_train, graphdef, state), infos = jax.block_until_ready(
            jax.lax.scan(train_step, (key_train, graphdef, state), batches, log_interval)
        )
        ...
        if (t + log_interval) % eval_interval == 0 or t == train_time_step - log_interval:
            fb_model, _, actor, _, _ = nnx.merge(graphdef, state)
            result = fb_awr.evaluate(actor, fb_model, dataset, num_inference_samples, render=True)
